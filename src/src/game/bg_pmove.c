...
#define PM_JUMP_DELAY 0   // ETMan: instant double jump (was 850)
...
static float PM_CmdScale(usercmd_t *cmd)
{
	...
	// ETMan: Disabled heavy weapon slowdown - all weapons move at full speed
	...
	// if (GetWeaponTableData(pm->ps->weapon)->skillBased == SK_HEAVY_WEAPONS && !CHECKBITWISE(GetWeaponTableData(pm->ps->weapon)->type, WEAPON_TYPE_MORZER | WEAPON_TYPE_SET))
	// {
	// 	...disabled...
	// }
...
/**
 * @brief PM_CheckDoubleJump - ETMan: Check and execute double jump if enabled
 * @return qtrue if double jumped
 */
static qboolean PM_CheckDoubleJump(void)
{
	// Double jump only works if enabled via g_misc flag
	if (!(pm->gameMiscFlags & G_MISC_DOUBLE_JUMP))
	{
		return qfalse;
	}

	// Already used double jump this airtime
	if (pm->pmext->doubleJumped)
	{
		return qfalse;
	}

	// Need to be pressing jump
	if (pm->cmd.upmove < 10)
	{
		return qfalse;
	}

	// ETMan: Must have released and re-pressed jump for double jump
	// Check if this is a NEW jump press (not held from first jump)
	if (pm->ps->pm_flags & PMF_JUMP_HELD)
	{
		return qfalse;
	}

	// Can't double jump while prone
	if (pm->ps->eFlags & EF_PRONE)
	{
		return qfalse;
	}

	// Execute the double jump - higher than first jump!
	pm->ps->pm_flags        |= PMF_JUMP_HELD;
	pm->ps->groundEntityNum  = ENTITYNUM_NONE;  // Ensure we're "leaving ground"
	pm->ps->velocity[2]      = JUMP_VELOCITY * 1.5f;   // ETMan: 50% higher than normal jump
	pm->pmext->doubleJumped  = qtrue;
...
static void PM_GroundTrace(void)
{
	...
	pm->pmext->airTime      = 0;
	pm->pmext->doubleJumped = qfalse;  // ETMan: reset double jump when landing
...
{
	// ETMan: Don't auto-switch for panzer - keep it equipped
...
static void PM_Weapon(void)
{
	...
	else
	{
		...
		{
			// ETMan: Removed panzer spinup - fires immediately now
			// pfaust has spinup time in MP (DISABLED)
			// if (GetWeaponTableData(pm->ps->weapon)->type & WEAPON_TYPE_PANZER)
			// {
			...
			// ETMan: Skip fire delay for panzer in panzer war mode
			if ((pm->gameMiscFlags & G_MISC_PANZER_WAR) &&
			    (GetWeaponTableData(pm->ps->weapon)->type & WEAPON_TYPE_PANZER))
			{
				pm->ps->weaponDelay = 0;  // No delay, fire immediately
			}
			else
			{
				pm->ps->weaponDelay = GetWeaponTableData(pm->ps->weapon)->fireDelayTime;
	...
	// ETMan: Skip ammo check for panzer in panzer war mode - infinite ammo
	if (GetWeaponTableData(pm->ps->weapon)->uses > PM_WeaponAmmoAvailable(pm->ps->weapon))
	{
		// In panzer war mode, panzers have infinite ammo - don't switch weapons
		if ((pm->gameMiscFlags & G_MISC_PANZER_WAR) &&
		    (GetWeaponTableData(pm->ps->weapon)->type & WEAPON_TYPE_PANZER))
		{
			// Don't send EV_NOAMMO - just continue firing
		}
		else
		{
			PM_AddEvent(EV_NOAMMO);
	...
	// ETMan: Skip ammo usage for panzer in panzer war mode
	if (PM_WeaponAmmoAvailable(pm->ps->weapon) != -1)
	{
		// check for being mounted
		if (!BG_PlayerMounted(pm->ps->eFlags))
		{
			// In panzer war mode, panzers don't use ammo
			if (!((pm->gameMiscFlags & G_MISC_PANZER_WAR) &&
			      (GetWeaponTableData(pm->ps->weapon)->type & WEAPON_TYPE_PANZER)))
			{
	...
	else
	{
		...
		// ETMan: Don't play WEAP_ATTACK_LASTSHOT for panzer - it covers the screen
		// because we're no longer auto-switching away after firing
...
void PM_Sprint(void)
{
	if (pm->waterlevel <= 1) // no sprint & no stamina recharge under water
	{
		...
		{
			// ETMan: Check for unlimited sprint via g_misc flag
			if (pm->ps->powerups[PW_ADRENALINE] || pm->ps->powerups[PW_NOFATIGUE]
			    || (pm->gameMiscFlags & G_MISC_UNLIMITED_SPRINT))
			{
		...
		else
		{
			...
			// ETMan: Also check for unlimited sprint flag
			if (pm->ps->powerups[PW_ADRENALINE] || pm->ps->powerups[PW_NOFATIGUE]
			    || (pm->gameMiscFlags & G_MISC_UNLIMITED_SPRINT))
			{
				pm->pmext->sprintTime = SPRINTTIME;
	...
	else if (GetWeaponTableData(pm->ps->weapon)->type & WEAPON_TYPE_SCOPED)
	{
		...
		else
		{
			...
			if (((!pm->pmext->airTime || (pm->ps->pm_flags & PMF_JUMP_HELD)) && !(pm->cmd.buttons & BUTTON_WALKING)
			     && VectorLength(pm->ps->velocity) > 127 && pm->cmd.serverTime > pm->pmext->switchToScopeTime + 200)
