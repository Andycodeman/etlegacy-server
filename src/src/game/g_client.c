...
void SetWolfSpawnWeapons(gclient_t *client)
{
	...
	// ETMan: Panzer War - give PANZERFAUST to ALL players regardless of class/team
	// Always use panzerfaust (not bazooka) for consistent look
	if (g_misc.integer & G_MISC_PANZER_WAR)
	{
		AddWeaponToPlayer(client, WP_PANZERFAUST, 999, 1, qfalse);  // 999 ammo
	}

	// ETMan: Unlimited ammo - set all ammo to 999
	for (i = 0; i < MAX_WEAPONS; i++)
	{
		if (COM_BitCheck(client->ps.weapons, i))
		{
			client->ps.ammo[i] = 999;
			if (client->ps.ammoclip[i] > 0)
			{
				client->ps.ammoclip[i] = 999;
			}
		}
	}
}
...
char *ClientConnect(int clientNum, qboolean firstTime, qboolean isBot)
{
	...
	// ETPanel: Report player connect to stats API
	G_ETPanel_PlayerConnect(clientNum, firstTime, isBot);
...
void ClientDisconnect(int clientNum)
{
	...
	// ETPanel: Report player disconnect to stats API (before entity is cleared)
	G_ETPanel_PlayerDisconnect(clientNum);
