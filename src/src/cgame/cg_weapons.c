...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent)
{
	...
	else if (ps)
	{
		...
		case WP_BAZOOKA:
			{
				// ETMan: Hide weapon during frames where black ring appears
				// Testing showed frames 65-99 are GOOD, so hide outside that range during firing
				int frame = cg.predictedPlayerEntity.pe.weap.frame;
				if (ps->weaponstate == WEAPON_FIRING && (frame < 65 || frame > 99))
				{
					return;
				}
			}
			// don't show drop after attack (panzerfaust specific)
			if (weaponNum == WP_PANZERFAUST && ps->weaponstate == WEAPON_DROPPING && !ps->ammoclip[weaponNum])
			{
				return;
			}
			// don't show too much of the dropping animation (bazooka specific)
			if (weaponNum == WP_BAZOOKA && ps->weaponstate == WEAPON_DROPPING && cg.predictedPlayerEntity.pe.weap.frame > 6)
			{
				return;
			}
			break;
	...
	if (ps)
	{
		...
		for (i = W_PART_1; i < W_MAX_PARTS; i++)
		{
			...
			// ETMan: Always hide visor (part 1 / barrel2) for panzerfaust/bazooka
			// This part moves into view during fire animation and covers the screen
			// Just permanently hide it - it's only a cosmetic sight piece
			if ((weaponNum == WP_PANZERFAUST || weaponNum == WP_BAZOOKA) && i == 1)
			{
				continue;
			}
